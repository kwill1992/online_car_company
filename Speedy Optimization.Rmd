...
$$X \in Integers$$
~where C is the cost from each city to Houston, where X is the calculated number of vehicles to move from each city to Houston, & where S is the number of vehicles shipped from each city to Houston.
$$x \ge 15$$
$$a_i \ge 0~~~\forall i$$
$$x \in A$$
...
#### Do it in R  
Below are results from this problem solved via MILP optimization in R.  
The first box shows that the solver found an optimal solution and what it is.  
The second box are the formal results.  
...
### Let's try an optimization between 10 cities and 2 hubs
The optimization problem looks like this:
...
Excel has solved this trivial problem at a cost of $xxxx. You can see cost to ship between the From and To cities as "Unit Cost". Supply/Demand has the number of vehicles that need to be shipped. "Ship" has the solved solution.  You can see now that there is a choice between two hubs.  
...
### Let's try an optimization between 10 cities and 2 hubs but we have to choose one or the other, not either one, i.e. only one hub will used
