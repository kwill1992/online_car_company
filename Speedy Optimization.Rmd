...
In the above, AK and HI are removed. There are 1,001 cities in this data base with populations greater than 40,000. Optimization of this many cites might doable for advanced commercial optimization algorithms, but it is far too many for what we want to do for this project and certainly too many for the Solver that comes with MS Excel.  
...
The second box has more formal results.  
...
Here is our first true optimization problem. We have 10 metro areas, the cost to ship between each and Houston or Washington and the number of cars to be shipped. Here we will let the solver chose which hub each city will ship to that will give us the least expensive solution. The optimization write-up looks like this:
$$min\sum_{i=1}^{10} \sum_{j=1}^{2}C_{i,j}X_{i,j}$$
$$st: \sum_{j=1}^{2}X_{i,j} \ge S_i~~~\forall i,~i=1~to~10,~~~\forall j,~j=1~to~2$$
...
![Small problem - 10 Cities, 2 hubs](Images/SCS 2C 2H.png)  
...
$$min\sum_{i=1}^{10} \sum_{j=1}^{2}C_{i,j}X_{i,j}$$
$$st: \sum_{j=1}^{2}X_{i,j} \ge S_i,~~~\forall i=1~to~10,~~~\forall j=1~to~2$$
$$ \sum_{j=1}^{2}Y_j=1,~~~\forall j=1~to~2$$
...
![Small problem - 10 Cities, 2 hubs, must choose ONLY 1 hub](Images/SCS 2 C Choose 1.png)  
...
$$st: \sum_{j=1}^{10}X_{i,j} \ge S_i,~~~\forall i,~i=1~to~10,~~~\forall j,~j=1~to~10$$
$$ \sum_{j=1}^{10}Y_j=NumHubs,~~~\forall j,~j=1~to~10$$
$$X_{i,j} \le MAX(S)*Y_j,~~~\forall i,~i=1~to~10,~~~\forall j,~j=1~to~10$$
$$X_{i,j} \ge 0~~~\forall i,~i=1~to~10,~~~\forall j,~j=1~to~10$$

$$X \in Integers$$
$$ Y \in 0~or~1$$
~where $C$ is the cost from each city to each hub, where $X$ is the calculated number of vehicles to move from each city to each hub, where $Y$ is which hubs were chosen in the solution, and where $S$ is the number of vehicles shipped from each city to a hub.  


#### Solution in Excel  
![Problem - 10 Cities, 10 hubs](Images/SCS 10 C cant do.png)  
...
The first box below has the optimal cost soltion. The second has the "hubs" solution: New York, Los Angeles, and Dallas were chosen. The third box shows the number of cars shipped between each city and each hub.  
...
```{r warning= FALSE, message = FALSE}
# Choose  two, one must be Houston
...
# Choose number of cities to use
num_cities <- 50
# filter. But does nothing here as we are using max, 50, cities.
city_data <- city_data %>% 
  group_by(num.from) %>%
  slice_head(n = num_cities) %>% 
  group_by(num.to) %>% 
  slice_head(n = num_cities) %>% 
  group_by(num.from)
# Get number of cars to ship
num_cars_month <- 4000
# Redo supply for each city based on the number of cars per month
get_supply <- get_supply %>% 
  slice_head(n = num_cities) %>% 
  mutate(to_pop_ratio = to_population / sum(to_population)) %>% # percent of Population
  mutate(to_num_cars = round(to_pop_ratio * num_cars_month,0)) # Calc number cars moving each month
# make into a Shipping cost matrix
cost <- city_data$cost
cost_6_city <- matrix(cost, nrow = num_cities, byrow = FALSE)
...
# Choose number of hubs to model
num_hubs <- 2
# Make model
model <- MIPModel()  %>% 
  # Number of cars shipped from Xi to Xj
  add_variable(x[i,j], i = 1:length(supply), j = 1:length(supply), type = "integer", lb = 0) %>% 
  # Chosing hubs variable
  add_variable(y[j], j = 1:length(supply), type = "binary") %>% 
  # minimize shipping cost
  set_objective(sum_expr(cost_6_city[i,j] * x[i,j], i = 1:length(supply), j = 1:length(supply)), "min") %>% 
  # must use supply from each city
  add_constraint(sum_expr(x[i, j], j = 1:length(supply)) >= supply[i], i = 1:length(supply)) %>% 
  # add this to keep Houston as a hub
  add_constraint(y[5] == 1) %>% 
  # Choose max humber of hubs
  add_constraint(sum_expr(y[j], j = 1:length(supply)) == num_hubs) %>% 
  # To link variables X and Y
  add_constraint(x[i,j] <= max(supply)*y[j], i = 1:length(supply), j = 1:length(supply))


#Run model and get detailed result but don't display
result <- solve_model(model, with_ROI(solver = "glpk", verbose = FALSE))
#Display optimal value
result
# Work on solution
...
We use the same 50 cities and let the Solver chose any three cities that will minimize cost.  
...
# Choose number of cities to use
num_cities <- 50
# filter out extra cities if using less than 50
city_data <- city_data %>% 
  group_by(num.from) %>%
  slice_head(n = num_cities) %>% 
  group_by(num.to) %>% 
...
# get number of cars to ship each month
num_cars_month <- 4000
...
library(ompr)
library(ompr.roi)
# Make number of cars to ship into a supply vector
supply <- as.vector(get_supply$to_num_cars)
# get input: number of hubs to model
num_hubs <- 3
...
model <- MIPModel()  %>% 
  ...
  # must use supply from each city
  add_constraint(sum_expr(x[i, j], j = 1:length(supply)) >= supply[i], i = 1:length(supply)) %>% 
  # Must use number of hubs 
  add_constraint(sum_expr(y[j], j = 1:length(supply)) == num_hubs) %>% 
  # add contraint linking variables X and Y
  add_constraint(x[i,j] <= max(supply)*y[j], i = 1:length(supply), j = 1:length(supply))

#Run model, get detailed results, but don't display
result <- solve_model(model, with_ROI(solver = "glpk", verbose = FALSE))
#Display optimal value
result
# Working nice solution
solution <- as_tibble(get_solution(result, x[i,j]))
# get hub solution
solution_hub <- as_tibble(get_solution(result, y[j]))
to.column <- as.vector(get_supply$to)
...
#Clean up solution, no 0's
solution <- solution %>%
  filter(value > 0)
# Work displaying solution
solution_display <- solution %>%
  filter(value > 0) %>%
  select(variable, i, j, value, FROM_city, TO_city)
#Display solution
...
They will be referred to as: "High" and "Low".  And the original "Med" shown below:
$$Cost(Dollars) = 100 + 100\sqrt{miles}$$  
...
~ where $B$ = sqft of building needed, $N_{cars}$ = number of cars per month/20, [rounded up to next whole number for each work day, 20 workdays per month].  
...
As it turns out, the 2015 Macbook used wasn't fully up to the task. Significantly more time was needed to solve problems with 50 metro areas and up to 50 hubs. 1 hub solved in about 1 minute. 2 hubs needed about 10 minutes. 3 hubs - 35 minutes. 4 hubs - well over an hour. For 5 hubs, the solver appeared to be about 40% complete at 3 hours and the calculation was stopped. It just wasn't reasonable to keep going.  
Fortunately, 25 metros and up to 25 hubs was significantly quicker. The total time for these smaller optimization runs was just over two hours.  
A plot of the outcomes with the different shippping costs and rent/labor costs is below.  

#### Plot output for 25 Cities and 1 to 25 Hubs
...
#### Main Observation  
Changing the cost of operating of the hubs is more of a factor than changing costs of shipping. [As modeled.]  
...
5) Include a much more accurate estimate of the rent/labor of a metro area in the actual optimization function. This is much more complex but could perhaps result in different outcomes. For example, if Philadelphia is much less expensive than New York, Philadelphia might be a hub chosen over New York even though New York had a lot more cars.  
